...
本文示例基于 **Lua 5.1——5.4** 编写，涵盖各版本的常用特性。需要注意以下版本差异：

- `table.maxn`、`module`、`setfenv`、`getfenv`、`loadstring` 等函数在 **Lua 5.2 及以后已移除或改版**，文中相关示例附有替代说明；
- `coroutine.running()` 从 Lua 5.2 起返回协程线程对象；
- `collectgarbage("setpause"|"setstepmul")` 等参数语义在不同版本略有出入。
...
流程图如下：
![while 循环]({{ site.baseurl }}/images/posts/lua-while.webp)
...
![repeat...until 循环]({{ site.baseurl }}/images/posts/lua-repeat-until.webp)
...
![if 语句]({{ site.baseurl }}/images/posts/lua-if.webp)
...
Lua认为false和nil为假，true 和非nil为真。要注意的是Lua中 0 为 true。
if 语句流程图如下：
![if...else 语句]({{ site.baseurl }}/images/posts/lua-if-else.webp) #####**_if...elseif...else 语句_**
...
![for 循环]({{ site.baseurl }}/images/posts/lua-for.webp)
